...
//Close keyboard after pressing inside text field and pressing enter
...
- (BOOL)textField: (UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString: (NSString *)string
{
    ...
    {
        myTextField.text = [myTextField.text stringByReplacingCharactersInRange:range withString:string];
        return NO;
    }
...
- (void)viewDidLoad
{
    ...
    //Displays keyboard when clicking inside the text field
    [myTextField becomeFirstResponder];
    
    //Load the output coded in 'initPlot' when the app starts
    //[self initPlot];
}
...
{
    NSNumber *myNumber = [NSNumber numberWithDouble:[myTextField.text doubleValue]];
    ...
    while (lambda<1.01)
    {
        ...
        dsyev_("N", "U", &n, X, &n, w, work, &lwork, &info);
        free(work);
...
- (IBAction)Go:(id)sender
{
    ...
    //Converts string that has been entered into the text field into a number (double) that can be used, ignoring any extra decimal points that could have been entered
    NSNumber *myNumber = [NSNumber numberWithDouble:[myTextField.text doubleValue]];
    
    //Displays in 'black box at bottom of this page'
    NSLog(@"You entered %@ and the number is %@", myTextField.text, myNumber);
    //Displays the characters entered into the text field as a string in the text view named 'output'
    output.text = [NSString stringWithFormat:@"You have entered %@ and the number is %@", myTextField.text, myNumber];
    
    //Call 'initPlot' and 'calculateEigevalues' when the go button is pressed
    [self initPlot];
